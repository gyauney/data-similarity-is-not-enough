...
# Everything needs to be saved by example so we can mix-and-match in eval
for percent in [25, 50, 75, 100]:
    for language in ['english', 'spanish', 'french', 'german', 'swahili']:
        if language == 'english' and percent != 100:
            continue
        print(language)
        dataset_name = f'xnli_{percent}-percent-{language}'
        with open(f'./{data_dir}/{dataset_name}-prompted.json', 'r') as f:
